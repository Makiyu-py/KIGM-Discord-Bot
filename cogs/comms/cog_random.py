...
class RandomCommands(commands.Cog, name=':grey_question: Randomness'):
    ...
    async def youtubecomment(self, ctx, *, comment):
        ...
        embed = discord.Embed(title="Here's your Comment!",
    ...
    async def jailify(self, ctx, user: Optional[discord.Member] = None):
        ...
        async with request("GET", Jailpfp, headers={}) as response:
            if response.status == 200:
                embed = discord.Embed(title=f":headstone: Rip cuz *{user}* is in *jail*",
                                      description=f"[avatar link]({Jailpfp})", color=self.bot.main_colour)
                embed.set_footer(text=f'{ctx.author} | Powered by https://some-random-api.ml/',
    ...
    @commands.command(aliases=['quotekanye'],
    ...
    async def kanyequote(self, ctx):
        ...
        async with request("GET", KanyeAPI, headers={}) as response:
            if response.status == 200:
                ...
                embed = discord.Embed(title="From the Words of Kanye West:", description=f'"{data["quote"]}"',
    ...
    async def wasted(self, ctx, user: Optional[discord.Member] = None):
        ...
        avatar = str(user.avatar_url).replace('.webp', '.png').replace('.gif', '.png').replace('?size=1024',
                                                                                               '?size=512')

        UFAPI = self.sr.filter("wasted", avatar)
        UFAPI = UFAPI.url

        embed = discord.Embed(title="w a s t e d .", description=f'[T h e  i m a g e  l i n k]({UFAPI})',
                              color=self.bot.main_color)
        embed.set_footer(text=f'{ctx.author} | Powered by https://some-random-api.ml/', icon_url=ctx.author.avatar_url)
    ...
    async def loremipsum(self, ctx, number_of_sentences: Optional[int] = 6):
        ...
        lorem_ipsum = lorem_ipsum.replace("b", "")
        embed = discord.Embed(title="Lorem Ipsum Generator", description=lorem_ipsum, color=0x656565,
    ...
    async def useless_fact(self, ctx):
        ...
        async with request("GET", UFAPI, headers={}) as response:
            if response.status == 200:
                ...
                embed = discord.Embed(title="Here's your Dose of a Useless Fact!", description=fact,
                                      color=self.bot.main_color)
                embed.set_footer(text=f'Requested By {ctx.author}', icon_url=ctx.author.avatar_url)
