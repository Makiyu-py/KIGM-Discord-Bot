...
class ModCommands(commands.Cog, name=':hammer_pick: Moderator Commands'):
  ...
  @commands.command(description='Delete a specified amount of messages! \n(only members with manage messages can access this command.)',aliases=['c', 'removem', 'removemessage', 'rmms'])
  @commands.guild_only()
  ...
  @commands.command(description='Mute a member! \n(only members with manage roles have access to this command)', aliases=['mutemember', 'muteuser'])
  @commands.guild_only()
  ...
  async def mute(self, ctx, user: discord.Member):
    ...
    if GuildData == False:
      await ctx.send("**ERROR!**You may have not set your muted role yet!\nYou may by using the `setmuted <muted_role>` command.")
      return
    ...
    if role in user.roles:
      ...
      try:
        ...
        await ctx.send(f"I am not allowed to mute {user.mention}!")
      else:
        await ctx.send(f"I have now muted {user.mention}!")
  ...
  async def unmute(self, ctx, user: discord.Member):
    ...
    try:
      ...
      await ctx.send("You may have not set your muted role!\nYou may by using the `setmuted <muted_role>` command.")
    else:
      try:
        await user.remove_roles(role)
      except discord.Forbidden:
        await ctx.send(f"I am not allowed to unmute {user.mention}!")
      else:
        if not role in user.roles:
          await ctx.send(f"{user} is **already unmuted!**")
        else:
          await ctx.send(f"I have now unmuted {user.mention}!")

  @commands.command(description='kick a member! \n(only members with kick members have access to this command)',aliases=['kickmember', 'k'])
  @commands.guild_only()
  ...
  @commands.command(description='Ban a member! \n(only members with ban members have access to this command.)',aliases=['banmember', 'b'])
  @commands.guild_only()
  ...
  @commands.command(description='Softban a user! \n(Basically just kicks and deletes all messages of a user, 5 second ban lmao)', aliases=['sb'])
  @commands.guild_only()
  @commands.has_permissions(ban_members = True)
  async def softban(self, ctx, user_mention: discord.Member):
    try:
      ...
      await ctx.send(f"**ERROR!**\nCannot softban {user_mention.mention}.\nReason: User is higher than the/as high as the bot to softban.")

  @commands.command(description='Unban a member! \n(only members with ban members have access to this command.)',aliases=['unbanmember', 'ub'])
  @commands.guild_only()
  @commands.has_permissions(ban_members = True)
