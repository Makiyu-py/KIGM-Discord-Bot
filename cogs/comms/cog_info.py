...
class InfoCommands(commands.Cog, name=':information_source: Informative Commands'):
    ...
    async def stats(self, ctx):
        ...
        embed.add_field(name="<:smolnitro:801662615553441792>Total Guilds I'm In", value=serverCount)
        ...
        embed.set_author(name=f"{self.bot.user.name} Stats", icon_url=self.bot.user.avatar_url)
    ...
    @commands.command(description="Get the profile picture of a specific member!",
    ...
    async def whois(self, ctx, member: Optional[discord.Member] = None):
        ...
        embed.add_field(name="Discord Name, Nickname", 
                        value=(member.name + ", " + member.display_name) if str(member.name) != str(member.display_name) else member.name, 
                        inline=True)
